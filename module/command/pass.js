module.exports = Pass;
Pass.prototype.toTgMessage = toTgMessage;
Pass.prototype.run = run;

function Pass() {
  this.permitAll = true;
}

async function run(context, password) {
  ...
  await context.api.store.addUser(context.user.id, getUserName(context.user));
